...
ssr_chara = [311001001, 311001008, 311001009, 311001010, 311001013, 311001014, 311001015, 311001020,
             ...
             396004001, 396005001]
ssr_effect_3 = [3,3,3,3,3,4,4,5,5,6,6,7,7,8]
...
def gacha_list():
    ...
    gacha_feature_chara_list = ssr_chara + sr_chara + r_chara
    #     [311001001,311001004,311001007,311001008,311001009,311001010,311001013,311001014,
...
def gacha_draw():
    ...
    for i in range(draw_count):
        rs = draw()
        if rs[0] in ssr_chara: max_effect = 3
        elif rs[0] in sr_chara: max_effect = 2
    ...
    pbrs.effect_1 = random.randint(1, max_effect)
    pbrs.effect_2 = random.randint(pbrs.effect_1, max_effect)
...
@bp.route("/rate_list", methods=["POST"])
def gacha_rate_list():
    pbrq = pb.RequestRateList()
    pbrs = pb.ResponseRateList()
    for i in range(5):
